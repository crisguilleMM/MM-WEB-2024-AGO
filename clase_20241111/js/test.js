...
/*
*Agregamos el addEventListener  DOMContentLoaded  
*para que corra cuando la pagina este cargada
*/
document.addEventListener("DOMContentLoaded", function () {
    const buttons = document.querySelectorAll('button');
    for (let i = 0; i < buttons.length; i++) {
        console.log(`Loop `+i);//se usa para leer variable 
        buttons[i].addEventListener('click', createParagraph);
        console.log(buttons[i],'Tiene : ' +buttons[i].value);
    }
});